...
export function clearCookies(res) {
  ...
  if (!providerConfig?.cookies?.long) {
    throw new Error('OIDC provider config must include cookies.long');
  }

  for (const name in providerConfig.cookies.names) {
    console.log('clearCookies', providerConfig.cookies.names[name]);
    res.clearCookie(providerConfig.cookies.names[name], {
      ...providerConfig.cookies.long
    });
  };
  console.log('clearCookies headers', res.getHeaders());
}
