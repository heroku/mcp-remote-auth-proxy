...
export function useSessionReset(app, authServerUrl, instanceConfig) {
  ...
  app.get(sessionResetUrl.pathname, (req, res, _next) => {
    clearCookies(res);
    res.redirect(sessionResetDoneUrl);
  });

  app.get(sessionResetDoneUrl.pathname, (req, res, _next) => {
...
export function clearCookies(res) {
  if (!providerInstanceConfig) {
    throw new Error(
      'useSessionReset(app, authServerUrl, providerInstanceConfig) must be called to initialize this module'
    );
  }
  for (const name in providerInstanceConfig.cookies.names) {
    res.clearCookie(
      providerInstanceConfig.cookies.names[name],
