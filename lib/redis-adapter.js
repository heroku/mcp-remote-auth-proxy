...
export function redisAdapterInit(env = {}) {
  const {
  ...
  client = new Redis(mcpAuthProxyRedisUrl.href, redisOpts);
...
class RedisAdapter {
  constructor(name) {
    if (!didInit) {
      throw new Error('redisAdapterInit(env) must be called during app start-up');
    }
    this.name = name;
  }
