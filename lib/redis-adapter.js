...
class RedisAdapter {
  constructor(name) {
    if (!RedisAdapter.client) {
      throw new Error('RedisAdapter.init(env, errorFunc) must be called during app start-up');
    }
    this.name = name;
  }
  ...
  static init(env = {}, errorFunc) {
    ...
    try {
      ...
      throw new Error(`MCP_AUTH_PROXY_REDIS_URL must be a valid URL, ${err}`);
    }
    ...
    if (typeof errorFunc !== 'function') {
      throw new Error(`RedisAdapter.init second param "errorFunc" must be a function`);
    }
    
    this.client = new Redis(mcpAuthProxyRedisUrl.href, redisOpts);
    this.client.on("error", errorFunc);
  }
