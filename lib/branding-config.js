...
function isHerokuProvider() {
  ...
  if (BRANDING_PROVIDER === 'heroku') {
    return true;
  }
  if (IDENTITY_SERVER_URL && IDENTITY_SERVER_URL.includes('heroku.com')) {
    return true;
  }
  if (IDENTITY_SERVER_URL && IDENTITY_SERVER_URL.includes('herokudev.com')) {
    return true;
  }
...
function getBrandingConfig() {
  const {
    BRANDING_TITLE = 'Login for Model Context Protocol',
    BRANDING_FAVICON
  } = process.env;
  ...
  if (isHeroku) {
    ...
    return {
      ...
      favicon: BRANDING_FAVICON || '/favicon.svg',
