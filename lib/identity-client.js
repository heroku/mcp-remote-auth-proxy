...
// Custom error classes for better token refresh error classification
class RefreshTokenExpiredError extends Error {
  constructor(message) {
    super(message);
    this.name = 'RefreshTokenExpiredError';
    this.isRecoverable = true; // Can restart OAuth flow
    this.shouldRetry = false;  // Don't retry, need re-auth
  }
}
...
class TokenRefreshError extends Error {
  constructor(message, originalError = null) {
    ...
    this.isRecoverable = false; // May be temporary network issue
    this.shouldRetry = true;    // Can potentially retry
...
async function refreshIdentityToken(authProxyProvider, authProxyClient) {
  ...
  try {
    ...
    if (err.error === 'invalid_grant' || err.error === 'invalid_token') {
      // Refresh token expired - need full re-authentication
      throw new RefreshTokenExpiredError(`Refresh token expired, full re-authentication required: ${err.message}`);
    } else if (err.code === 'ECONNREFUSED' || err.code === 'ETIMEDOUT' || err.code === 'ENOTFOUND') {
      // Network issues - might be temporary
      throw new TokenRefreshError(`Network error during token refresh: ${err.message}`, err);
    } else if (err.status >= 500 && err.status < 600) {
      // Server errors - might be temporary
      throw new TokenRefreshError(`Server error during token refresh (${err.status}): ${err.message}`, err);
    } else {
      // Other error - unknown issue, don't retry
      throw new TokenRefreshError(`Token refresh failed: ${err.message}`, err);
    }
  }
}
...
export { 
  ...
  RefreshTokenExpiredError, // Error classification classes
  TokenRefreshError,
