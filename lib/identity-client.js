...
export async function identityClientInit(env = {}) {
  ...
  try {
    const _IDENTITY_SCOPE_parsed = IDENTITY_SCOPE.split(new RegExp('[, ]+'));
  ...
  if (IDENTITY_SERVER_METADATA_FILE && IDENTITY_SERVER_METADATA_FILE != '') {
    try {
      ...
      logger.error('Error reading IDENTITY_SERVER_METADATA_FILE', {
        error: err.message,
        description:
          'Should be a JSON file containing OpenID Provider Metadata for the configured IDENTITY_* provider (only required if the identity provider does not directly support OpenID Connect Discovery 1.0)',
      });
      throw err;
    }
  } else {
    try {
      ...
      logger.error('Error using OpenID Connect Discovery', {
        ...
        description:
          'which should return OpenID Provider Metadata (alternatively, write the metadata in a local JSON file, path configured IDENTITY_SERVER_METADATA_FILE)',
      });
      throw err;
    }
  }
