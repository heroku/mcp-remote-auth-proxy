...
import { getSessionResetUrl, clearCookies } from "./use-session-reset.js";
...
export default (app, provider) => {
  ...
  app.get(identityUniqueCallbackPath, setNoCache, async (req, res, next) => {
    try {
      ...
      // Clear session cookie, because Auth is successful, and to allow other MCP CLients to perform auth successfully using the same browser
      clearCookies(res);

      await provider.interactionFinished(req, res, result);
      console.log('interactionFinished headers', res.getHeaders());
    } catch (err) {
      return next(err);
    }
  });
  ...
  app.use((err, req, res, next) => {
    if (err instanceof SessionNotFound || err instanceof AccessDenied) {
      console.log(`${req.method} ${req.path} Resetting session because of ${error.name}, ${error.message}`);
