...
export default function useMcpServerProxy(app, provider, mcpServerUrl) {
  app.use(mcpServerUrl.pathname, async (req, res, next) => {
    ...
    // Proactively check and refresh token if needed (configurable)
    try {
      await checkAndRefreshToken(provider, proxyClient);
    } catch (err) {
      // Log but don't fail the request - reactive refresh will handle it if needed
      logger.debug('Proactive token refresh check failed, proceeding with request', {
        error: err.message,
        identityAuthId: proxyClient.identityAuthId
      });
    }
            ...
            case 401: {
              ...
              try {
                ...
                if (err instanceof RefreshTokenExpiredError) {
                  logger.error('proxy request token refresh failed - refresh token expired', { 
                    error: err.message,
                    errorType: err.name,
                    identityAuthId: proxyClient.identityAuthId,
                  ...
                  logger.error('proxy request token refresh failed - retryable error', { 
                    error: err.message,
                    errorType: err.name,
                    identityAuthId: proxyClient.identityAuthId,
                    isRecoverable: err.isRecoverable,
                  ...
                  logger.error('proxy request token refresh failed - unclassified error', { 
                    error: err.message,
                    identityAuthId: proxyClient.identityAuthId 
                  });
                }
