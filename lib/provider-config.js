...
const { PROVIDER_SCOPE = 'openid offline_access', OIDC_PROVIDER_JWKS } = process.env;
...
try {
  ...
  throw new Error(
    `PROVIDER_SCOPE must contain a string of space or comma separated scopes (error: ${err}`
  );
}
...
try {
  ...
  throw new Error(
    `OIDC_PROVIDER_JWKS must contain a JSON array of one or more private JSON Web Keysets (error: ${err}`
  );
}

export default {
  adapter: TokenRedisAdapter,
  extraClientMetadata: {
    properties: [
  ...
  extraParams: {
    ...
    scope: async function (ctx, value, _client) {
