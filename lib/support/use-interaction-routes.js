...
export default (app, provider) => {
  ...
  app.get(IDENTITY_CALLBACK_PATH, setNoCache, async (req, res, next) => {
    try {
      ...
      console.log(IDENTITY_CALLBACK_PATH, 'redirecting to', next_url)
  ...
  app.get(IDENTITY_UNIQUE_CALLBACK_PATH, setNoCache, async (req, res, next) => {
    try {
      ...
      console.log(IDENTITY_UNIQUE_CALLBACK_PATH, 'Token Endpoint Response', tokens);

      res.status(202).send('Auth successful, but the next step is under construction.')
