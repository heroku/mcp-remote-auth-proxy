...
const createStorageHook = (provider) => ({
  ...
  async storePKCEState(interactionId, state, codeVerifier, expiresAt) {
    ...
    if (typeof expiresAt !== 'number' || !Number.isFinite(expiresAt) || expiresAt <= 0) {
      logger.warn(
        'Invalid expiresAt provided for PKCE state; expected a positive numeric timestamp',
        {
          interactionId,
          expiresAt,
        }
    ...
    try {
      ...
      if (!interaction) {
        ...
        if (isFallbackStorageDisabled()) {
          logger.error(
            'Fallback storage is disabled in production - PKCE state cannot be stored without interaction',
            {
              interactionId,
          ...
          throw new Error(
            'Cannot store PKCE state: no interaction found and fallback storage is disabled in production'
          );
        }
      ...
      if (!clientId) {
        ...
        if (isFallbackStorageDisabled()) {
          logger.error(
            'Fallback storage is disabled in production - PKCE state cannot be stored without client_id',
            {
              interactionId,
              environment: process.env.NODE_ENV,
            }
          ...
          throw new Error(
            'Cannot store PKCE state: interaction missing client_id and fallback storage is disabled in production'
          );
        }
      ...
      if (client) {
        ...
        if (isFallbackStorageDisabled()) {
          logger.error(
            'Fallback storage is disabled in production - PKCE state cannot be stored without client',
            {
              interactionId,
              clientId,
              environment: process.env.NODE_ENV,
          ...
          throw new Error(
            'Cannot store PKCE state: no client found and fallback storage is disabled in production'
          );
        }
...
/**
 * Try to retrieve PKCE code verifier from fallback storage
 * @param {string} lookupKey - The key to look up in pkceStateStore
 * @param {string} clientId - Client ID for logging
 * @param {string} logContext - Context label for logging (e.g., 'interactionId' or 'state')
 * @returns {string|null} The code verifier if found and not expired, null otherwise
 */
function tryGetCodeVerifierFromFallback(lookupKey, clientId, logContext) {
  if (!lookupKey || isFallbackStorageDisabled()) {
    return null;
  }

  const fallbackData = pkceStateStore.get(lookupKey);
  if (fallbackData && fallbackData.expiresAt > Date.now()) {
    const codeVerifier = fallbackData.codeVerifier;
    pkceStateStore.delete(lookupKey);
    logger.debug(`Retrieved PKCE code verifier from fallback storage using ${logContext}`, {
      [logContext]: lookupKey,
      clientId,
    });
    return codeVerifier;
  }
...
async function exchangeIdentityCode(
  ...
  try {
    ...
    if (!codeVerifier && interactionId) {
      codeVerifier = tryGetCodeVerifierFromFallback(
        interactionId,
    ...
    // Skip if identityAuthState equals interactionId to avoid duplicate lookup
    const stateKey = authProxyClient.identityAuthState;
    if (!codeVerifier && stateKey && stateKey !== interactionId) {
      codeVerifier = tryGetCodeVerifierFromFallback(stateKey, authProxyClient.clientId, 'state');
    }
    ...
    if (!codeVerifier) {
      logger.error('No code verifier found', {
        ...
        hasInFallback:
          !isFallbackStorageDisabled() && interactionId ? pkceStateStore.has(interactionId) : false,
        hasStateInFallback:
          !isFallbackStorageDisabled() && authProxyClient.identityAuthState
