...
import { fromEnvironmentAsync, DefaultLogger, LogLevel } from '@heroku/oauth-provider-adapters-for-mcp';
...
export async function identityClientInit(env = {}, provider = null) {
  ...
  try {
    IDENTITY_SCOPE_parsed = IDENTITY_SCOPE ? IDENTITY_SCOPE.split(new RegExp('[, ]+')) : ['openid', 'profile', 'email'];
...
export function getOidcAdapter() {
  return oidcAdapter;
}
