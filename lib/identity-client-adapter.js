...
async function generateIdentityAuthUrl(
  ...
  try {
    ...
    // Verify PKCE state was stored (either in client session or fallback)
    const fallbackStored = pkceStateStore.has(interactionId);
    const freshClient = await authProxyProvider.Client.find(authProxyClient.clientId);
    const clientStored = !!freshClient?.identityAuthCodeVerifier;

    if (!fallbackStored && !clientStored) {
      // This shouldn't happen but log a warning if it does
      logger.warn('PKCE state not found in either client session or fallback storage', {
        interactionId,
    ...
    logger.debug('Generated identity auth URL', {
      ...
      clientId: authProxyClient.clientId,
      redirectUrl,
      pkceStoredInClient: clientStored,
 ...
 */
async function exchangeIdentityCode(authProxyProvider, authProxyClient, code, redirectUrl, interactionId = null) {
  if (!oidcAdapter) {
    throw new Error('identityClientInit(env) must be called during app start-up');
  }
  ...
  try {
    ...
    // If not in client session, check fallback storage using interactionId or state
    ...
    // Also try with identityAuthState as the key (state = interactionId in adapter)
    if (!codeVerifier && authProxyClient.identityAuthState) {
      const fallbackData = pkceStateStore.get(authProxyClient.identityAuthState);
      if (fallbackData && fallbackData.expiresAt > Date.now()) {
        codeVerifier = fallbackData.codeVerifier;
        pkceStateStore.delete(authProxyClient.identityAuthState);
        logger.debug('Retrieved PKCE code verifier from fallback storage using state', {
          state: authProxyClient.identityAuthState,
          clientId: authProxyClient.clientId,
        });
      }
    }
    ...
    if (!codeVerifier) {
      logger.error('No code verifier found', {
        ...
        hasInFallback: interactionId ? pkceStateStore.has(interactionId) : false,
        hasStateInFallback: authProxyClient.identityAuthState ? pkceStateStore.has(authProxyClient.identityAuthState) : false,
        fallbackStoreSize: pkceStateStore.size,
        interactionId,
        clientId: authProxyClient.clientId,
      });
