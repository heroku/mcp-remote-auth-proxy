...
if (typeof MCP_SERVER_RUN_COMMAND !== 'string') {
  console.log('MCP_SERVER_RUN_COMMAND must be a shell command');
  process.exit(1);
}
let mcpServerRunArgs;
try {
  mcpServerRunArgs = JSON.parse(MCP_SERVER_RUN_ARGS_JSON);
  if (!Array.isArray(mcpServerRunArgs)) {
    throw new Error('parsed into wrong type');
  }
} catch (err) {
  console.log('MCP_SERVER_RUN_ARGS_JSON must be a valid JSON array', err);
  process.exit(1);
}
if (typeof MCP_SERVER_RUN_DIR !== 'string') {
  console.log('MCP_SERVER_RUN_DIR must be a directory path');
  process.exit(1);
}
let mcpServerRunEnv;
try {
  mcpServerRunEnv = JSON.parse(MCP_SERVER_RUN_ENV_JSON);
  if (typeof mcpServerRunEnv !== 'object' || Array.isArray(mcpServerRunEnv) || mcpServerRunEnv == null) {
    throw new Error('parsed into wrong type');
  }
} catch (err) {
  console.log('MCP_SERVER_RUN_ENV_JSON must be a valid JSON object', err);
  process.exit(1);
}
