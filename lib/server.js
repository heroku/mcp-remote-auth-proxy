...
let makeMcpServerProxy = function(mcpUrl, accessToken, accessTokenScope, dynamicClientId) {
  return proxy(mcpUrl.origin, {
    ...
    proxyReqOptDecorator: function(proxyReqOpts, _srcReq) {
      ...
      if (accessTokenScope) {
        proxyReqOpts.headers['x-authorization-scope'] = accessTokenScope;
      }
      if (dynamicClientId) {
        proxyReqOpts.headers['x-dynamic-client-id'] = dynamicClientId;
      }
      return proxyReqOpts;
    }
  })
...
app.use(mcpServerUrl.pathname, async (req, res, next) => {
  ...
  makeMcpServerProxy(
    mcpServerUrl, 
    proxyClient['identityAuthAccessToken'], 
    proxyClient['identityAuthScope'],
    proxyClient.clientId
  )(req, res, next);
