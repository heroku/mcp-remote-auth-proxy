...
import TokenRedisAdapter from './token-redis-adapter.js';
import { initializeIdentityClient, setupInteractionRoutes } from './server-adapter-integration.js';
...
function server(env = {}, listeningCallback, exitFunc) {
  const {
    MCP_AUTH_PROXY_VERIFY_RELEASE = 'false',
  ...
  TokenRedisAdapter.init(process.env, (message) => {
  ...
  // Initialize identity client asynchronously (non-blocking)
  let identityClientReady = initializeIdentityClient(process.env, provider);
  ...
  if (OIDC_PROVIDER_VIEWS_PATH) {
    if (fs.existsSync(OIDC_PROVIDER_VIEWS_PATH)) {
      ...
      throw new Error(
        `The configured OIDC_PROVIDER_VIEWS_PATH does not exist, ${OIDC_PROVIDER_VIEWS_PATH}`
      );
    }
  } else {
  ...
  app.get('/.well-known/oauth-authorization-server', async (_req, res) => {
    res.json({
      issuer: `${BASE_URL}`,
      authorization_endpoint: `${BASE_URL}${providerConfig.routes.authorization}`,
      token_endpoint: `${BASE_URL}${providerConfig.routes.token}`,
      token_introspection_endpoint: `${BASE_URL}${providerConfig.routes.introspection}`,
      userinfo_endpoint: `${BASE_URL}${providerConfig.routes.userinfo}`,
      jwks_uri: `${BASE_URL}${providerConfig.routes.jwks}`,
      registration_endpoint: `${BASE_URL}${providerConfig.routes.registration}`,
      scopes_supported: providerConfig.scopes,
  ...
  // Declare the Protected Resource Metadata for OAuth 2.0 clients
  ...
  setupInteractionRoutes(app, provider);
  ...
  const appListenFunc = async (mcpServerProcess) => {
    // Wait for identity client to be ready before starting server
    await identityClientReady;
    ...
    // Once this point is reached, the server launched successfully.
    // Use this var in Release Phase process, to verify the release is not misconfigured.
    if (MCP_AUTH_PROXY_VERIFY_RELEASE === 'true') {
      logger.info('Verified release launches. Exit status 0. (MCP_AUTH_PROXY_VERIFY_RELEASE=true)');
      exitFunc(0);
    }
  };
  try {
