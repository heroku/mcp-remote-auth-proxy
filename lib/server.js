...
import { identityClient, identityProviderMetadata } from './support/identity-client.js';
...
providerConfig.findAccount = Account.findAccount;
...
const baseInteractionPolicy = interactionPolicy.base();
baseInteractionPolicy.remove('consent');
...
let makeMcpServerProxy = function(mcpUrl, accessToken) {
  return proxy(mcpUrl.origin, {
...
app.use(mcpServerUrl.pathname, async (req, res, next) => {
  ...
  if (!proxyClient?.identity_auth_access_token) {
  ...
  accessTokenForRequest = proxyClient.identity_auth_access_token;

  // Check if Identity access token is valid with the primary auth provider
  let userInfo = await identityClient.fetchUserInfo(
    identityProviderMetadata, 
    proxyClient.identity_auth_access_token, 
    proxyClient.identity_auth_id
  );
  if (!userInfo) {
    // Perform refresh to get new access token
    let refreshTokenGrant = await identityClient.refreshTokenGrant(
      identityProviderMetadata, 
      proxyClient.identity_auth_refresh_token
    );
    if (!refreshTokenGrant) {
      res.set('WWW-Authenticate', 'Bearer error="invalid_token", error_description="Invalid identity refresh');
      res.status(401).json({
        error: 'invalid_token',
        error_description: "Invalid identity refresh"
      });
      return;
    }

    // Save fresh access token in Client model
    proxyClient['identity_auth_access_token'] = refreshTokenGrant.access_token;
    proxyClient['identity_auth_signature'] = refreshTokenGrant.signature;
    proxyClient['identity_auth_scope'] = refreshTokenGrant.scope;
    proxyClient['identity_auth_token_type'] = refreshTokenGrant.token_type;
    proxyClient['identity_auth_issued_at'] = refreshTokenGrant.issued_at;
    await provider.Client.adapter.upsert(proxyClient.clientId, proxyClient.metadata());

    accessTokenForRequest = refreshTokenGrant.access_token;
  }

  console.log('/mcp authorized', proxyClient.identity_auth_id);
  makeMcpServerProxy(mcpServerUrl, accessTokenForRequest)(req, res, next);

  // temporary WIP response
  // res.status(501).json({
  //   error: 'incomplete_implmentation',
  //   error_description: "work-in-progress"
  // });
});
