...
describe('Branding Configuration', function () {
  ...
  describe('isHerokuProvider', function () {
    it('should return true when IDENTITY_SERVER_URL contains heroku.com', function () {
      ...
      assert(
        isHerokuProvider(),
        'Should detect Heroku when IDENTITY_SERVER_URL contains heroku.com'
      );
    });

    it('should return true when IDENTITY_SERVER_URL contains herokudev.com', function () {
      process.env.IDENTITY_SERVER_URL = 'https://identity.example-herokudev.com';
      assert(
        isHerokuProvider(),
        'Should detect Heroku when IDENTITY_SERVER_URL contains herokudev.com'
      );
    });
  ...
  describe('getBrandingConfig', function () {
    it('should return Heroku branding when Heroku is detected', function () {
      ...
      assert(
        config.favicon === 'https://www.herokucdn.com/favicon.ico',
        'Should use Heroku favicon'
      );
      assert(config.colors.primary === '#8363a1', 'Should use Heroku primary color');
      assert(
        config.colors.background.includes('linear-gradient'),
        'Should use Heroku gradient background'
      );
    });
    ...
    it('should return generic branding when Heroku is not detected', function () {
      ...
      assert(
        config.colors.background.includes('linear-gradient'),
        'Should use generic gradient background'
      );
    });
    ...
    it('should use custom BRANDING_FAVICON when provided', function () {
      ...
      assert(
        config.favicon === 'https://example.com/custom-favicon.ico',
    ...
    it('should append Heroku to custom title when Heroku is detected', function () {
      ...
      assert(
        config.title === 'Custom Auth Service | Heroku',
        'Should append Heroku to custom title'
      );
    });
  });
});
